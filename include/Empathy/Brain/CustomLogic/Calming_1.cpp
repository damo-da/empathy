...
void empathy::brain::Calming_1::runLineNumber(std::string lineID, std::string callerID, cJSON *overrideData) {
    ...
    if (lineID == CALMING_COLLECT_NAME) {
        ...
        showtext->setDuration(1.f);
        showtext->setColor(0.8, 0.5, 0.5);
        showtext->setFadeOutDuration(1.f);
        ...
        if (true) {
            ...
            showtext->setDuration(3.0f);
            showtext->setFadeOutDuration(0.1f);
            showtext->setFadeInDuration(0.2f);
            showtext->setColor(0.8f, 0.5f, 0.5f);
            addLifeEvent(showtext);
            ...
            showtext2->setColor(0.8, 0.5, 0.5);
            addLifeEvent(showtext2);


            activateTimeoutForNextLine("p1.4", 6.0);
...
void empathy::brain::Calming_1::onReceiveEvent(empathy::radio::Event &event) {
    ...
    if (event.action == CALMING_COLLECT_NAME) {
        ...
        activateTimeoutForNextLine(CALMING_ACCESS_NAME, 2.0f);
        activateTimeoutForNextLine("p1.3", 2.f);
